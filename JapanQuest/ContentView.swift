...
struct QuestCameraView: View {
    ...
    private var cameraControls: some View {
        VStack(spacing: 13) {
            if let previewImage {
                previewControls(previewImage: previewImage)
            } else {
                captureControls

                Text("\(cameraModeText)から撮影し、続けて反対側を撮ります")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(.white.opacity(0.48))
            }
        }
    ...
    private func previewControls(previewImage: UIImage) -> some View {
        HStack(spacing: 12) {
            Button {
                resetCapture()
            } label: {
    ...
    private var captureControls: some View {
        HStack(spacing: 0) {
            ...
            Button {
                ...
                ZStack {
                    Circle()
                        .stroke(isUnlocked ? .white : .white.opacity(0.25), lineWidth: 5)
...
struct AccountView: View {
    ...
    private var profileHeader: some View {
        VStack(spacing: 14) {
            ...
            HStack(spacing: 10) {
                AccountStat(title: "今週", value: "\(memoryStore.visibleFeedPosts().filter { $0.isMine }.count)")
                AccountStat(title: "フレンド", value: "\(friendStore.friends.count)")
                AccountStat(title: "スポット", value: "\(myMemoryCount)")
            }
        }
    }
    ...
    private var profileContent: some View {
        VStack(spacing: 12) {
            AccountMenuRow(icon: "pencil", title: "プロフィールを編集")
            AccountMenuRow(icon: "qrcode", title: "QRコード")
            AccountMenuRow(icon: "bell.fill", title: "通知")
            AccountMenuRow(icon: "lock.fill", title: "公開範囲")
            AccountMenuRow(icon: "gearshape.fill", title: "設定")
        }
    }
    ...
    private var addFriendContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("今はローカルの仮追加です。Firebase導入後に、本当のユーザー検索と申請に変えます。")
                .font(.system(size: 12, weight: .medium))
                .foregroundStyle(.white.opacity(0.42))
                .lineSpacing(3)
        }
...
struct AccountSectionButton: View {
    let title: String
    let section: AccountSection
    @Binding var selectedSection: AccountSection
    ...
    var body: some View {
        HStack(spacing: 13) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                Text(friend.recentPlace)
                    .font(.system(size: 12, weight: .bold))
                    .foregroundStyle(.white.opacity(0.75))

                Text(friend.lastSharedText)
        ...
        VStack(spacing: 13) {
            HStack(spacing: 13) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("@\(request.username) ・ \(request.requestedAtText)")
                        .font(.system(size: 12, weight: .medium))
                        .foregroundStyle(.white.opacity(0.48))
                }
...
struct AccountQuickMenuView: View {
    @EnvironmentObject var friendStore: QuestFriendStore
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                AccountMenuRow(icon: "person.badge.plus", title: "フレンド追加")
                AccountMenuRow(icon: "person.2.fill", title: "フレンド一覧")
                AccountMenuRow(icon: "bell.fill", title: "リクエスト \(friendStore.incomingRequests.count)")
                AccountMenuRow(icon: "gearshape.fill", title: "設定")
