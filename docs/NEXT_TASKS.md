...
**変更対象:** `HomeView.swift` / `MapView.swift` / `CameraView.swift`（各画面 1 ファイルずつ）

**確認・改善すべき項目（例）:**
- Home: 「最近のシェア」→「フレンドの記録」/ 「最近埋まった場所」→「保存した場所」などセクション名の改善（Step 3-C）
- Map: スポット未選択時の空状態・ロード演出の有無
- Camera: 撮影後の体験・エラー時フォールバック表示の確認

**ユーザーに見える変化:** 各画面の文言・レイアウト・空状態の質が上がる。「整ったUI」から「記憶に残る体験」へ近づく。

**リスク:** 低〜中（1 画面・1 ファイルずつ進めれば安全）

**ピクトリ独自性:** 中〜高。コアフロー上のすべての画面が高品質でないとピクトリの世界観が壊れる。

**推奨度:** ★★★（候補 A または B と並行して進めやすい）
